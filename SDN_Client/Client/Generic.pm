...
sub new {
  ...
  $self->{requests_per_second}        = $self->{args}->{requests_per_second}    || 0;
...
sub _throttle {
  my $self = shift;
  return if $self->{requests_per_second} < 1;
  my $redis = Redis->new();
  return unless ref $redis;
  my $call_cnt = $self->_incr($redis);
  my $start_time = time();
  while (($call_cnt > $self->{requests_per_second}) && ((time()-$start_time) < $self->{request_max_queue_time})) {
    my $sleep = 600000 + int(rand(100000));
    NetMRI::LoggerShare::logDebug("_throttle: requests per second limit exceeded for key [" 
                                  . $self->get_throttle_key() 
                                  . "] (requests per second count: ${call_cnt}, max: " . $self->{requests_per_second} . "), sleeping for $sleep microseconds");
    usleep($sleep);
    $call_cnt = $self->_incr($redis);
  }

  NetMRI::LoggerShare::logDebug("_throttle: exit cycle [" 
                                  . $self->get_throttle_key() 
                                  . "] requests per second count: ${call_cnt}, max: " . $self->{requests_per_second});
}

sub _incr {
  my ($self, $redis) = @_;
  my $key = $self->get_throttle_key() . '/' . time();
