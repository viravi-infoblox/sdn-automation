...
sub test_velocloud_sdn {
  $log->info('Testing connection to VeloCloud');
  unless ($username && $password) {
    $log->error('username and password are required');
    $log->info($error_string);
    return;
  }
  my $params = {username => $username, password => $password, fabric_id => 1};
  $params->{ca_cert} = $ca_cert if ($ca_cert && $protocol eq 'https');
  add_common_params($params);
  push @addresses, '' unless scalar(@addresses);
  foreach my $address (@addresses) {
    $params->{address} = $address if $address;
    my $api_helper = NetMRI::HTTP::Client::VeloCloud->new(%$params);
    my $client     = NetMRI::SDN::VeloCloud->new(
      sql               => $sql,
      logger            => $log,
      api_helper        => $api_helper,
      network_interface => $network_interface || ''
    ...
    if ($network_interface && !$client->{local_interface_conf}) {
      ...
      $log->info('Performing request'
          . ($address ? " to $protocol://$address" : '')
          . ($client->{local_interface_conf} ? ' using ' . $client->{local_interface_conf} : ''));
      my ($res, $error) = $client->getApiClient()->v2_get_enterprises();
      $log->debug("Received from VeloCloud API:");
      $log->debug(Dumper($res));
      if ($res) {
        $log->info($ok_string);
      } else {
