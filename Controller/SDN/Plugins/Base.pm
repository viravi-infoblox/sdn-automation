...
sub build_mapping {
    ...
    my $table  = $sdndev->target_table();

    $sql->execute("CREATE TEMPORARY TABLE tmp_keys ($src_key VARCHAR(255), INDEX($src_key)) ENGINE=InnoDB");

    my $batch_size = 1000;
    my @batch;
    foreach my $record (@$data) {
        next unless defined $record->{$src_key};
        push @batch, $record->{$src_key};
        if (@batch >= $batch_size) {
            my $values = join(", ", map { "(" . $sql->escape($_) . ")" } @batch);
            $sql->execute("INSERT IGNORE INTO tmp_keys VALUES $values");
            @batch = ();
        }
    }
    if (@batch) {
    ...
    $sql->table(
        "SELECT SdnControllerId, d.${src_key}, d.${dest_key}
         FROM $table AS d
         JOIN tmp_keys AS t ON d.${src_key} = t.${src_key}",
        Callback => sub {
            my $row = shift;
    ...
    $sql->execute("DROP TEMPORARY TABLE tmp_keys");
